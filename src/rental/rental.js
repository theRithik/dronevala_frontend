...
const data2  ={
   ...
   const {data} =  useQuery({
    ...
    useEffect(()=>{
        ...
        setProducttype(Fc)
        const Ct2 = data.map((item,i)=>{
            return item.droneCategory
        })
        const Fc2 = Ct2.filter((item,i)=>{
            return Ct2.indexOf(item)===i
        })
        // console.log(Fc2,Ct2)
        setDronetype(Fc2)
    ...
    if(data){
        if(data.length>0){
            return data.map((item)=>{
                let nm = item.brand.replace(/\s+/g, '-');
                let cu = item.productName.replace(/\s+/g, '-');
                return(
                    <Link to={`/rental/${nm}/${cu}/${item.rentalID}`} key={item.id} >
                    <div className="acdcard">
                        {item.image ?(
 <img src={`${endpoints.imageprefix}${item.image}`} className="card5img" alt="academy"/>
...
<div>
    ...
    <p>₹ {item.price} <span style={{fontSize:8,fontWeight:500,color:"grey"}}>Price per day</span></p>
    ...
    <i class="bi bi-suit-heart likeicon"></i>
            ...
            description={
              <h6 style={{fontSize:12,fontWeight:700}}> No Product Found</h6>
            }
...
const val = acad2
   ...
   const res= val.filter((item)=>{
        return item.productCategory.toLowerCase().indexOf(dt.toLowerCase())>-1
    })
    setAcad(res)
}
...
const hidemenu=(e)=>{
    ...
    return(
        ...
        <Helmet>
        <title>Rental - Dronevala</title>
        <link rel="Dronevala" href="https://dronevala.com/rental"/>
        <meta name="description" content=" Explore our vast selection of high-quality drones available for rent, perfect for aerial photography, filming, real estate, and commercial projects. With our user-friendly online platform, you can easily browse, select, and rent the perfect drone to suit your needs. Our drones feature state-of-the-art technology and advanced cameras to ensure stunning visuals and reliable performance. Enjoy competitive pricing, flexible rental packages, and professional support." />
        <link rel="canonical" href="https://dronevala.com/rental" />
        <meta name="keywords" content="drone rentalDrone rental,Rent a drone,Drone hire,Drone rental services,Aerial photography drone rental,UAV rental,Professional drone rental,Drone camera rental,Drone leasing,Drone rental near me,Commercial drone rental,Drone rental for events,Rent a drone for photography,Drone rental company,Affordable drone rental,Drone rental for filming,Drone rental prices,Drone rental packages,Drone rental for real estate,Drone rental business"/>
        <meta property="og:title" content="Dronevala - Academy" />
        <meta property="og:url" content="https://dronevala.com/rental" />
        <meta property="og:image" content="https://dronevala.com/images/logo.png" />
        <meta property="og:type" content="website" />
        <meta property="og:description" content=" Explore our vast selection of high-quality drones available for rent, perfect for aerial photography, filming, real estate, and commercial projects. With our user-friendly online platform, you can easily browse, select, and rent the perfect drone to suit your needs. Our drones feature state-of-the-art technology and advanced cameras to ensure stunning visuals and reliable performance. Enjoy competitive pricing, flexible rental packages, and professional support."/>
    </Helmet>
...
<div >
    ...
    <h3>Rental </h3>
    <p>Explore our vast selection of high-quality drones available for rent</p>
    </div>
    <hr style={{color:'#b1adad'}}/>
    <div style={{display:'flex',marginTop:20}}>
        {acad.length>0 &&
<h6 className="Pcount">{acad.length} Products</h6>
}{ acad.length<1 &&
<h6 className="Pcount">Products</h6>
}
...
<span className="menuover"></span>
    ...
    <span className="menubutton menuclickcou" onClick={showAll}>All</span>
        {producttypeRender(producttype)}
        </div>
</div>
...
<div style={{marginTop:20}}>
        <div className="menuElement2" onClick={hidemenu}>
    <span className="menuover"></span>
    <h4> Drone Cateogry</h4>
    <i class="bi bi-chevron-up arrowup"></i>
    <i class="bi bi-chevron-down arrowdown"></i>
    </div>
    <div className="submenuElements">
{servicetypeRender(dronetype)}
    </div>
    </div>
}
    <hr style={{color:'#9b9797',marginTop:30}}/>
