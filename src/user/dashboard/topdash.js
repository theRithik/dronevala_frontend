...
import UserLeftdash from "./leftdash";
...
const dt={
  endpoint:endpoints.userprofilephoto
}
  const {data} = useQuery({
...
const menurender=(data)=>{
  if(data){
    ...
    return data.map((item)=>{
      ...
      <Link key={item.id} style={{color:'inherit'}} onClick={handclick} to={`/user/${item.link}`}>
      <li>
  <i class={`bi bi-${item.icon}`}>
    ...
    return(
      ...
      <hr/>
      <Link to="/user/support" style={{color:'inherit'}}>
      <li>
  ...
  <Link to="/user/orders" style={{color:'inherit'}}>
  <li>
  <i class="bi bi-box"></i>
  <p>orders</p>
  </li>
  </Link>
  <Link to="/user/settings" style={{color:'inherit'}}>
  <li>
...
{menurender(search)}
   ...
   <div style={{position:'relative'}}>
    {image &&
   <img src={`${endpoints.imageprefix}${image}`} onClick={imgClick} style={{width:40,height:40,cursor:'pointer',borderRadius:'50%'}} alt="person"/>
    }{!image &&
