...
const dt4={
    endpoint:endpoints.vendorRentalOrders,
}
const {data:ren}=useQuery({
    queryKey:['vrentalorders'],
    queryFn:()=>VGetData(dt4)
})
    ...
    const rentalApi=async(data)=>{
       try{
        if(data?.data){
            const ft  = await data.data
            setRental(ft.reverse())
            for(let i = 0;i<ft.length;i++){
                // const dt = data.data[i].DateandTime.split(',')[0]
                //     const dt2 = new Date(dt.split('/').reverse().join('/'))
                //     const dt3 = dt2.toString().slice(0,15)
        ...
        if(data){
            if(data.length>=1){
                return data.map((item)=>{
                    ...
                    return(
                        <div key={item.id} style={{marginTop:'30px'}}>
                 <Link to={`/vendor/rentalorder/${item.id}`} style={{color:'black'}}>
                    <div className=""  id="Aodisplay">
                        {item.status!=='seen' &&
                        <div className="circle2" id={item.id}>
                        </div>
